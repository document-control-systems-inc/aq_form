...
public class UserController {
	...
	@RequestMapping(value = "/loginAquarius", method = RequestMethod.GET, headers = "Accept=application/json")
	public ServiceResponse loginAquarius(@RequestHeader(required = false, value = "user") String user, 
			@RequestHeader(required = false, value = "password") String password) {
		ServiceResponse response = new ServiceResponse();
		response.setStatus(-1);
		try {
			JsonNode aquariusResponse = ldapService.checkPasswordAquarius(user, password);
			if (aquariusResponse != null) {
				if (aquariusResponse.findValue("access_token") != null &&
						aquariusResponse.findValue("access_token").asText() != null) {
					String token = aquariusResponse.findValue("access_token").asText();
					UserSession session = new UserSession();
					session.setUserUid(user);
					session.setHostnameConnected("localhost");
					if (!ldapService.validateSession(session)) {
						session = ldapService.createSessionAquarius(session, token);
	...
	@RequestMapping(value = "/user/info", method = RequestMethod.GET,headers="Accept=application/json")
	public ServiceResponse getUserInfo(@RequestHeader(required = false, value = "token") String token, 
		...
		try {
			...
			session.setHostnameConnected("localhost");
			if (ldapService.validateSession(session)) {
				response.setExito(ldapService.getUserInfo(uid));
				if (response.getExito() != null) {
					response.setStatus(0);
				} else {
					response.setStatus(101);
