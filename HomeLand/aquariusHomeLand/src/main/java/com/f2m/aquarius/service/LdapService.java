...
public class LdapService {
	...
	private AquariusLdapParameters aquariusConnectionParams;
	private SessionParams sessionParams;
	private GeneralUtils gutils = new GeneralUtils();
	private UserService userService = new UserService();
	private WsPostService wsPost = new WsPostService();
	...
	public LdapService() {
		...
		aquariusConnectionParams = ctx.getBean(AquariusLdapParameters.class);
		ctx.close();
	}
	...
	public UserSession createSessionAquarius(UserSession userSession, String token) throws Exception {
		validateSessionParams(userSession);
		Date actualTime = new Date();
	...
	public JsonNode checkPasswordAquarius(String user, String password) throws Exception {
		if (user == null || user.length() == 0 || password == null
				|| password.length() == 0) {
			throw gutils.throwException(105, "");
		}
		String url = aquariusConnectionParams.getProtocol() + "://" + aquariusConnectionParams.getHostname();
		if (aquariusConnectionParams.getPort() > 0) {
			url += ":" + aquariusConnectionParams.getPort();
		}
		url += "/" + aquariusConnectionParams.getContext();
		url += "/token";
		...
		return wsPost.sendPost(url, jsonArgs);
	}
	...
	public List<User> getUserInfo(String uid) throws Exception {
