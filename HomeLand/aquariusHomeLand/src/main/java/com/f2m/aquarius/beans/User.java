...
public class User {
	...
	private List<String> formsView;
	private List<String> formsCreate;
	...
	public List<String> getFormsView() {
		return formsView;
	}
	public void setFormsView(List<String> formsView) {
		this.formsView = formsView;
	}
	
	public void addFormsView(String dn) {
		if (formsView == null) {
			formsView = new ArrayList<String>();
		}
		if (!formsView.contains(dn)) {
			formsView.add(dn);
		}
	}
	
	public Boolean removeFormsView(String dn) {
		Boolean response = false;
		if (formsView != null) {
			int indexOf = formsView.indexOf(dn);
			if (indexOf > -1) {
				formsView.remove(indexOf);
				response = true;
			}
		}
		return response;
	...
	public List<String> getFormsCreate() {
		return formsCreate;
	}
	public void setFormsCreate(List<String> formsCreate) {
		this.formsCreate = formsCreate;
	}
	
	public void addFormsCreate(String dn) {
		if (formsCreate == null) {
			formsCreate = new ArrayList<String>();
		}
		if (!formsCreate.contains(dn)) {
			formsCreate.add(dn);
		}
	}
	
	public Boolean removeFormsCreate(String dn) {
		Boolean response = false;
		if (formsCreate != null) {
			int indexOf = formsCreate.indexOf(dn);
			if (indexOf > -1) {
				formsCreate.remove(indexOf);
				response = true;
			}
		}
		return response;
