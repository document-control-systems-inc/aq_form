	...
	public User getUserAditionalInfo(User user) throws AquariusException {
		...
		if (userJson != null) {
			if (userJson.size() > 1) {
				...
				if (userJson.get(0).findValue("givenName") != null) {
					user.setGivenName(userJson.get(0).findValue("givenName").asText());
				}
				if (userJson.get(0).findValue("lastName") != null) {
					user.setLastName(userJson.get(0).findValue("lastName").asText());
				}
				JsonNode groups = userJson.get(0).findValue("group");
				if (groups != null) {
					if (groups.isArray()) {
						for (JsonNode group:groups) {
							user.addGroup(group.asText());
						}
					}
				}
				JsonNode formsCreates = userJson.get(0).findValue("formsCreate");
				if (formsCreates != null) {
					if (formsCreates.isArray()) {
						for (JsonNode formsCreate:formsCreates) {
							user.addFormsCreate(formsCreate.asText());
						}
					}
				}
				JsonNode formsViews = userJson.get(0).findValue("formsView");
				if (formsViews != null) {
					if (formsViews.isArray()) {
						for (JsonNode formsView:formsViews) {
							user.addFormsView(formsView.asText());
						}
					}
				}
