...
import com.f2m.aquarius.parameters.AquariusLdapParameters;
...
public class ConfigurationService {
	...
	private AquariusLdapParameters aquariusLdap;
	private FormsParameters forms;
	...
	public AquariusLdapParameters getLdapAquarius() {
		try {
			List<JsonNode> obj = db.selectJson("configuration", "data->>'id' = 'ldapAquarius'", null);
			if (!obj.isEmpty()) {
				aquariusLdap = mapper.readValue(obj.get(0).toString() , AquariusLdapParameters.class);
			}
		} catch (AquariusException e) {
			logger.error("Error al obtener la configuración de LDAP: " + e.getMessage());
			e.printStackTrace();
	...
	public FormsParameters getForms() {
		try {
			List<JsonNode> obj = db.selectJson("configuration", "data->>'id' = 'forms'", null);
			if (!obj.isEmpty()) {
				forms = mapper.readValue(obj.get(0).toString() , FormsParameters.class);
			}
		} catch (AquariusException e) {
			logger.error("Error al obtener la configuración de Forms: " + e.getMessage());
			e.printStackTrace();
		} catch (Exception e) {
			logger.error("Error al obtener la configuración de Forms: " + e.getMessage());
			e.printStackTrace();
	...
	public JsonNode getemailAutomaticFile() {
		try {
			List<JsonNode> obj = db.selectJson("configuration", "data->>'id' = 'emailAutomatic'", null);
			if (!obj.isEmpty()) {
				return obj.get(0);
			}
		} catch (AquariusException e) {
			logger.error("Error al obtener la configuración de los envios automáticos de email: " + e.getMessage());
			e.printStackTrace();
		} catch (Exception e) {
			logger.error("Error al obtener la configuración de los envios automáticos de email: " + e.getMessage());
			e.printStackTrace();
